...
unsigned int OPL_Pico_PortRead(opl_port_t port)
{
    ...
    {
        // 0x00 = OPL3 present. 0xFF = not present (OPL2 only).
        return 0x00;
    }
...
void OPL_Pico_WriteRegister(unsigned int reg_num, unsigned int value)
{
    switch (reg_num)
    {
        ...
        default:
            ...
            {
                // OPL3 mode: if writing to a 0xC0 channel register (feedback/algorithm/panning)
                // and no output bits are set (bits 4-7 all zero), default to both L+R enabled.
                // This fixes games like KQ6 that write 0xC0 regs without panning bits, which
                // would cause channels to silently disappear once newflag (reg 0x105) is set.
                uint8_t reg_low = reg_num & 0xFF;
                if (reg_low >= 0xC0 && reg_low <= 0xC8 && (value & 0xF0) == 0)
                    value |= 0x30; // enable L1 + R1
                s_chip.write_address(reg_low);
            }
#else
